...
router.delete('/:id', auth, async(req, res) => {
  try {
    ...
    //check on the user
    if(post.user.toString() !== req.user.id) res.status(401).json({msg:`NOT AUTHORIZED`})
...
router.delete('/comment/:id/:comment_id', auth, async(req, res) => {
  try {
    ...
    // make sure comment doesnt come back false
    if(!comment) return res.status(404).json({msg: `Comment does not exist.`})
    //Check if the user who made the comment is the one deleting
